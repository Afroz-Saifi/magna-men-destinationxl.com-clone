...
function displayOrders(data) {
  ...
  data.forEach((element2) => {
    ...
    if (element2.orders.length > 0) {
      orderingUsers.push(element2);

      let tr = document.createElement("tr");
      tr.setAttribute("class", "trClass")

      let td1 = document.createElement("td");
      td1.innerText = element2.id;
      let td2 = document.createElement("td");
      td2.innerText = element2.fullname;
      let td3 = document.createElement("td");
      let mem = "";

      for (let i = 0; i < element2.orders.length - 1; i++) {
        mem += element2.orders[i].name + " , "
      }
      mem += element2.orders[element2.orders.length - 1].name;
console.log(element2.orders[0].name)
      td3.innerText = mem;
      let td4 = document.createElement("td");
      td4.innerText = element2.estimatedDate;
      td4.addEventListener("click", () => {
        let user = usersList.filter((el) => el.id == element2.id)
        console.log(user)
      ...
      let sum = 0;
      for (let i = 0; i < element2.orders.length; i++) {
        sum += +element2.orders[i].price;
      }
      td5.innerText = "₹" + sum + "/-";
      let td6 = document.createElement("td");
      td6.innerText = element2.orderStatus;
      if (element2.orderStatus == "Completed") {
        td6.style.color = "green";
      } else if (element2.orderStatus == "Dispatch Initiated") {
        td6.style.color = "blue";
      } else if (element2.orderStatus == "Dispatch Pending") {
        td6.style.color = "red";
      } else if (element2.orderStatus == "Return/Exchange Requested") {
      ...
      td7.innerText = element2.paymentMode;
